...
export function AddAccountModal({ onClose }: AddAccountModalProps) {
	...
	async function handleAdd() {
		...
		if (!cleanToken) {
			setError(
				"No access token detected — paste an access token, or a token JSON file that includes one.",
			);
			return;
		}
		setError("");
		...
		try {
			...
			if (!resp.ok) {
				...
				throw new Error(
					`API error ${resp.status}: ${errText.substring(0, 100)}`,
				);
			}
			...
			toast(
				"Account added: " +
					acc.email +
					(acc.refreshToken ? " (auto-refresh on)" : ""),
				"success",
			);
			onClose();
		} catch (e) {
			...
			setError(
				err.name === "AbortError"
					? "Request timed out. Check your internet or token."
					: err.message,
			);
		} finally {
			setLoading(false);
	...
	async function handleAddWithRefreshOnly() {
		...
		if (!cleanRefresh) {
			setError(
				"No refresh token detected — paste a refresh token, or a token JSON file that includes one.",
			);
			return;
		}
		...
		if (!pUrl) {
			setError(
				"Refresh proxy URL is required (browser blocks direct call to Microsoft). On Vercel this is already deployed at /api/refresh — enter that (or your own proxy URL) and click Save proxy URL.",
			);
			return;
		}
		...
		try {
			...
			if (!currentAcc || currentAcc.accessToken === PLACEHOLDER)
			...
			const resp = await fetch(
				"https://graph.microsoft.com/v1.0/me?$select=displayName,mail,userPrincipalName",
				{
					headers: { Authorization: "Bearer " + currentAcc.accessToken },
					signal: controller.signal,
				},
			);
			clearTimeout(timeout);
			if (!resp.ok) {
				...
				throw new Error(
					resp.status === 401
						? "Token expired or invalid after refresh"
						: `API error ${resp.status}: ${errText.substring(0, 80)}`,
				);
			}
			const me = (await resp.json()) as {
				userPrincipalName?: string;
			...
			selectAccount(idx);
			toast(
				"Account added and refreshed: " + (me.userPrincipalName || me.mail),
				"success",
			);
			onClose();
		} catch (e) {
			...
			setError(
				(e as Error).message ||
					"Refresh failed. Check refresh token, Client ID, and Tenant ID.",
			);
		} finally {
			...
			<div className="modal-body">
				...
				>
					...
					<button
						...
						onClick={() => {
							if (proxyUrl.trim()) {
								saveRefreshProxyUrl(proxyUrl.trim());
